...
void loadVoxelConfig(ros::NodeHandle &nh, VoxelMapConfig &voxel_config)//从 ROS 参数服务器加载体素地图的配置参数
{
  ...
  nh.param<bool>("local_map/map_sliding_en", voxel_config.map_sliding_en, true);
...
void VoxelOctoTree::init_plane(const std::vector<pointWithVar> &points, VoxelPlane *plane)//根据输入的点云数据初始化一个平面
{
  ...
  evalsReal = evals.real();// 对特征值进行排序，确定最小特征值、最大特征值和中间特征值  特征值排序逻辑
...
void VoxelOctoTree::init_octo_tree()//初始化八叉树节点，判断当前节点是否构成平面，并根据结果决定是否继续分割
{
  if (temp_points_.size() > points_size_threshold_)//如果当前节点的点数 temp_points_ 超过阈值 points_size_threshold_，则进行平面初始化  检查当前八叉树节点内的点云数量是否超过预设阈值
  {
    ...
    if (plane_ptr_->is_plane_ == true)//是平面的时候
    {
      ...
      {//并检查点数是否超过最大点数 max_points_num_。如果超过，则禁用更新并清空点云
        update_enable_ = false;//不更新
        std::vector<pointWithVar>().swap(temp_points_);// 清空点云
        new_points_ = 0;//新的点云为0
      }
    }
    else//如果当前节点不构成平面，设置节点状态为 1，并调用 cut_octo_tree 函数继续分割
    {
      octo_state_ = 1;
      cut_octo_tree();//使用下一个构建的函数 使用条件是当不是平面的时候
    }
    init_octo_ = true;// 设置 init_octo_ 为 true
    new_points_ = 0;//设置 init_octo_ 为 true，并重置 new_points_ 为 0
  }
}
/*
八叉树  用于在当前节点不构成平面时，递归地将点云数据分割到八叉树的子节点中
*/
void VoxelOctoTree::cut_octo_tree()//在上一个函数中引用的 当前节点不构成平面时 使用此函数
{
  ...
  */
    ...
    {
      leaves_[leafnum] = new VoxelOctoTree(max_layer_, layer_ + 1, layer_init_num_[layer_ + 1], max_points_num_, planer_threshold_);//新树
      ...
      leaves_[leafnum]->voxel_center_[2] = voxel_center_[2] + (2 * xyz[2] - 1) * quater_length_;//中心
      leaves_[leafnum]->quater_length_ = quater_length_ / 2;//边长
    }
    // 将点加入子节点
  ...
  for (uint i = 0; i < 8; i++)
  {
    if (leaves_[i] != nullptr)//如果子节点的点数超过阈值，则初始化子节点并判断是否为平面
    {
      if (leaves_[i]->temp_points_.size() > leaves_[i]->points_size_threshold_)
      {
        ...
        if (leaves_[i]->plane_ptr_->is_plane_)//如果是平面
        {
          leaves_[i]->octo_state_ = 0;// 标记为平面
          // new added
          if (leaves_[i]->temp_points_.size() > leaves_[i]->max_points_num_)//并根据点数决定是否禁用更新和清空点云
          {
            leaves_[i]->update_enable_ = false;// 禁用更新 太多就不更新了
            std::vector<pointWithVar>().swap(leaves_[i]->temp_points_);// 清空点云 此时也清空点云
            new_points_ = 0;
          }
        }
        else//如果不是平面则继续分割
...
void VoxelOctoTree::UpdateOctoTree(const pointWithVar &pv)//更新八叉树节点，将新点插入到合适的节点中，并根据条件重新初始化或分割节点
{
  if (!init_octo_)//如果节点未初始化，则将新点加入 temp_points_，并检查是否需要初始化
  {
    ...
    if (temp_points_.size() > points_size_threshold_) { init_octo_tree(); }//如果点数超过阈值 points_size_threshold_，则调用 init_octo_tree 进行初始化
  }
  else
  {
    if (plane_ptr_->is_plane_)//如果当前节点是平面且允许更新，则将新点加入 temp_points_，并检查是否需要重新初始化平面或清空点云
    {
      if (update_enable_)//是否更新 必须更新
      {
        new_points_++;
        temp_points_.push_back(pv);
    ...
    else//如果当前节点不是平面且未达到最大层数，则将新点分配到子节点
    {
      if (layer_ < max_layer_)//最大层数
      {
        ...
        {//如果子节点不存在，则创建新的子节点，并设置其中心点和边长
          ...
          leaves_[leafnum]->UpdateOctoTree(pv);//递归调用 UpdateOctoTree 更新子节点
        }
      }
      else
      {
...
VoxelOctoTree *VoxelOctoTree::find_correspond(Eigen::Vector3d pw)//根据输入点 pw 的坐标，在八叉树中找到对应的节点
{
  ...
  int xyz[3] = {0, 0, 0};
  //根据点 pw 的坐标与当前节点中心点 voxel_center_ 的比较，确定点所在的子节点索引 leafnum
...
//如果当前节点不是平面节点且未达到最大层数，则根据点 pv 的坐标与当前节点中心点 voxel_center_ 的比较，确定点所在的子节点索引 leafnum
  if (init_octo_ && (!plane_ptr_->is_plane_) && (layer_ < max_layer_))//三个条件同时满足
  {
    int xyz[3] = {0, 0, 0};
    ...
    else//如果子节点不存在，则创建新的子节点并插入点
    {
      ...
      leaves_[leafnum]->voxel_center_[2] = voxel_center_[2] + (2 * xyz[2] - 1) * quater_length_;
      leaves_[leafnum]->quater_length_ = quater_length_ / 2;
      return leaves_[leafnum]->Insert(pv);//递归插入
    }
  }
...
*/
  for (size_t i = 0; i < feats_down_body_->size(); i++)//这段代码遍历 feats_down_body_ 中的每个点，计算其协方差矩阵并将其转换到机体坐标系
  {
    V3D point_this(feats_down_body_->points[i].x, feats_down_body_->points[i].y, feats_down_body_->points[i].z);
    if (point_this[2] == 0) { point_this[2] = 0.001; }//如果点的 z 坐标为 0，将其设置为 0.001，以避免计算错误
    M3D var;
    calcBodyCov(point_this, config_setting_.dept_err_, config_setting_.beam_err_, var);// 计算点的协方差矩阵
    body_cov_list_.push_back(var);// 将协方差矩阵存入 body_cov_list_
    point_this = extR_ * point_this + extT_;// 将点从 LiDAR 坐标系转换到机体坐标系
    M3D point_crossmat;
    point_crossmat << SKEW_SYM_MATRX(point_this);// 计算点的反对称矩阵
    cross_mat_list_.push_back(point_crossmat);// 将反对称矩阵存入 cross_mat_list_
  }
/*初始化状态估计变量
...
*/
  ...
  int rematch_num = 0;
  MD(DIM_STATE, DIM_STATE) G, H_T_H, I_STATE;//数据类型
...
*/
  bool flg_EKF_inited, flg_EKF_converged, EKF_stop_flg = 0;//后面用到的标志位
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    在下一个函数中进行了调用
    */
   //调用 TransformLidar 函数，将点从机体坐标系转换到世界坐标系
    ...
    for (size_t i = 0; i < feats_down_body_->size(); i++)//遍历每个点
    {
      pointWithVar &pv = pv_list_[i];
      //将点的机体坐标 point_b 和世界坐标 point_w 存入 pv_list_
      ...
      //计算点的协方差矩阵 cov，并存入 pv.var 和 pv.body_var
    ...
    ptpl_list_.clear();//清空 ptpl_list_，用于存储点-平面匹配结果
    ...
    */
    ...
    {
      total_residual += fabs(ptpl_list_[i].dis_to_plane_);//遍历 ptpl_list_ 中的每个点，累加其到平面的距离绝对值，得到总残差
    }
    effct_feat_num_ = ptpl_list_.size();//有效特征点的数量
    //输出原始特征点数量、下采样后的特征点数量、有效特征点数量及平均残差
    ...
    fast-lio里面的代码
    */
    MatrixXd Hsub(effct_feat_num_, 6);//计算测量雅可比矩阵
    MatrixXd Hsub_T_R_inv(6, effct_feat_num_);//和测量协方差矩阵
    VectorXd R_inv(effct_feat_num_);//初始化测量协方差向量 
    VectorXd meas_vec(effct_feat_num_);//更新测量向量
    meas_vec.setZero();
    for (int i = 0; i < effct_feat_num_; i++)
    {
      auto &ptpl = ptpl_list_[i];
      V3D point_this(ptpl.point_b_);
      point_this = extR_ * point_this + extT_;//将点从机体坐标系转换到世界坐标系
      V3D point_body(ptpl.point_b_);
      M3D point_crossmat;//计算点的反对称矩阵
      point_crossmat << SKEW_SYM_MATRX(point_this);

      /*** get the normal vector of closest surface/corner ***/
      /*** 获取最近表面/角点的法向量 ***/
      V3D point_world = state_propagat.rot_end * point_this + state_propagat.pos_end;//世界坐标系
      Eigen::Matrix<double, 1, 6> J_nq;//构建测量雅可比矩阵 
      ...
      // point_body cov 计算点的测量协方差矩阵 var，考虑旋转和平移的不确定性
      var = state_propagat.rot_end * extR_ * ptpl_list_[i].body_cov_ * (state_propagat.rot_end * extR_).transpose();

      //计算测量协方差 sigma_l，包括平面协方差和点的协方差
      double sigma_l = J_nq * ptpl_list_[i].plane_var_ * J_nq.transpose();
      //更新测量协方差向量 R_inv，考虑测量协方差和点的协方差
      ...
      //A 是通过点的反对称矩阵 point_crossmat 和旋转矩阵
      //state_.rot_end 以及法向量 ptpl_list_[i].normal_ 计算得到的
      Hsub.row(i) << VEC_FROM_ARRAY(A), ptpl_list_[i].normal_[0], ptpl_list_[i].normal_[1], ptpl_list_[i].normal_[2];
      //计算测量雅可比矩阵 Hsub 和其转置与测量协方差矩阵的乘积 Hsub_T_R_inv
      //使用 VEC_FROM_ARRAY(A) 将向量 A 转换为数组形式
      //将 ptpl_list_[i].normal_ 的三个分量添加到 Hsub 的一行中
      ...
      //更新测量向量 meas_vec，存储点到平面的距离
      meas_vec(i) = -ptpl_list_[i].dis_to_plane_;
    }

    //扩展卡尔曼滤波更新  计算卡尔曼增益矩阵 K  更新状态向量 state_
    EKF_stop_flg = false;
    flg_EKF_converged = false;//俩标志位
    /*** Iterative Kalman Filter Update ***/
    MatrixXd K(DIM_STATE, effct_feat_num_);//计算卡尔曼增益矩阵 
    // auto &&Hsub_T = Hsub.transpose();
    auto &&HTz = Hsub_T_R_inv * meas_vec;//计算测量雅可比矩阵的乘积
    // fout_dbg<<"HTz: "<<HTz<<endl;
    //计算测量雅可比矩阵的乘积H_T_H
    H_T_H.block<6, 6>(0, 0) = Hsub_T_R_inv * Hsub;//转置与测量协方差矩阵的乘积 Hsub_T_R_inv     计算测量雅可比矩阵 Hsub
    ...
    VD(DIM_STATE)
    //计算状态更新量 solution，包括旋转增量 rot_add 和平移增量 t_add
    solution = K_1.block<DIM_STATE, 6>(0, 0) * HTz + vec.block<DIM_STATE, 1>(0, 0) - G.block<DIM_STATE, 6>(0, 0) * vec.block<6, 1>(0, 0);
    int minRow, minCol;
    //更新状态向量 state_
    state_ += solution;
    auto rot_add = solution.block<3, 1>(0, 0);
    auto t_add = solution.block<3, 1>(3, 0);//r t
    //判断旋转增量 rot_add 和平移增量 t_add 是否小于预设阈值。如果满足条件，则设置 flg_EKF_converged 为 true
    if ((rot_add.norm() * 57.3 < 0.01) && (t_add.norm() * 100 < 0.015)) { flg_EKF_converged = true; }
    V3D euler_cur = state_.rot_end.eulerAngles(2, 1, 0);//计算当前状态的欧拉角 euler_cur

    /*** Rematch Judgement ***/
    //如果 EKF 已收敛或接近最大迭代次数，则增加重新匹配次数
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == config_setting_.max_iterations_ - 1)))
    {
      ...
      //如果 EKF 未停止且重新匹配次数达到要求或达到最大迭代次数，则更新状态协方差矩阵 state_.cov
      ...
      //更新最后位置 position_last_ 和四元数 geoQuat_
      ...
      EKF_stop_flg = true;//设置 EKF_stop_flg 为 true
    }
    if (EKF_stop_flg) break;//如果 EKF_stop_flg 为 true，则终止迭代
  }
...
void VoxelMapManager::BuildVoxelMap()//构建体素地图
{
  ...
  float planer_threshold = config_setting_.planner_threshold_;//平面的点云阈值
  int max_layer = config_setting_.max_layer_;//最大层数
  int max_points_num = config_setting_.max_points_num_;//一层中最大点数
  ...
  for (uint i = 0; i < plsize; i++)//将每个点分配到对应的体素中
  {
    ...
    ////遍历 input_points 中的每个点，计算其所在的体素位置
...
V3F VoxelMapManager::RGBFromVoxel(const V3D &input_point)//根据体素位置生成 RGB 颜色。
{
  ...
  for (int j = 0; j < 3; j++)
  {
    loc_xyz[j] = floor(input_point[j] / config_setting_.max_voxel_size_);//将输入点 input_point 的每个坐标除以体素大小  得到体素位置 loc_xyz
  }

  VOXEL_LOCATION position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);//计算输入点所在的体素位置
  int64_t ind = loc_xyz[0] + loc_xyz[1] + loc_xyz[2];//计算体素位置的索引 ind
  uint k((ind + 100000) % 3);
...
void VoxelMapManager::UpdateVoxelMap(const std::vector<pointWithVar> &input_points)//更新体素地图
{
  ...
  for (uint i = 0; i < plsize; i++)//对每个输入点，计算其所在的体素位置
  {
    ...
    //如果体素已存在，则调用 UpdateOctoTree 更新体素；否则，创建新的体素并更新
    //如果体素不存在，则创建新的体素，并调用 UpdateOctoTree 更新体素
    else
    {
      VoxelOctoTree *octo_tree = new VoxelOctoTree(max_layer, 0, layer_init_num[0], max_points_num, planer_threshold);
      voxel_map_[position] = octo_tree;
    ...
    if (current_layer < max_layer)//递归搜索八叉树的子节点，直到达到 max_layer
    {
      for (size_t leafnum = 0; leafnum < 8; leafnum++)
      {
        if (current_octo->leaves_[leafnum] != nullptr)
        {
          ...
          build_single_residual(pv, leaf_octo, current_layer + 1, is_sucess, prob, single_ptpl);//递归调用
        }
      }
      return;
    }
  ...
  if (current_octo->layer_ < current_octo->max_layer_)
  {
    if (!current_octo->plane_ptr_->is_plane_)
    {
      for (size_t i = 0; i < 8; i++)//8个部分的点云
      {
        ...
        { GetUpdatePlane(current_octo->leaves_[i], pub_max_voxel_layer, plane_list); }//迭代函数
      }
    }
  }
  return;
...
//判断当前位置 position_last_ 是否超出内存点大小，如果是，则更新地图
//跑其他数据集用到了 维护内存的函数
void VoxelMapManager::mapSliding()//地图滑动
{
  ...
  double t_sliding_start = omp_get_wtime();//并行计算时间
  float loc_xyz[3];//获取当前坐标点
  for (int j = 0; j < 3; j++)
  {
