...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<bool>("evo/pose_output_en", pose_output_en, true);//是否启用位姿输出，默认值为 true
  //nh.param<bool>("evo/pose_save_en", pose_save_en, true);//是否启用位姿保存，默认值为 false
  //nh.param<bool>("evo/pose_save_txt_en", pose_save_txt_en, true);//是否启用位姿保存为 txt 文件，默认值为 false
...
*/
  ...
  调试与发布
  */
  nh.param<double>("debug/plot_time", plot_time, -10);//绘图时间，默认值为 -10
...
*/
  ...
  if (pose_output_en)//将优化后的位姿（位置+四元数）写入 seq_name.txt，用于后续评测（如EVO工具）
  {
    ...
    evoFile << LidarMeasures.last_lio_update_time << " " << _state.pos_end[0] << " " << _state.pos_end[1] << " " << _state.pos_end[2] << " "
            << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << std::endl;//保存格式分别是时间戳、位置、四元数
  }
  ...
  */
  ...
  if(voxelmap_manager->config_setting_.map_sliding_en)
  {
    ...
    */
    voxelmap_manager->mapSliding();// 移除超出滑动窗口的区域/new add/
  }
  ...
  *pcl_w_wait_pub = *laserCloudWorld; // 缓存待发布点云
  ...
  if (voxelmap_manager->config_setting_.is_pub_plane_map_) voxelmap_manager->pubVoxelMap();//pubVoxelMap为发布的
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  if (pcd_save_en)//点云地图保存模块
  {
    ...
    *pcl_wait_save += *laserCloudWorldRGB;
    ...
    if (pcl_wait_save->size() > 0 && pcd_save_interval > 0 && scan_wait_num >= pcd_save_interval)//按间隔保存点云到指定路径，记录位姿信息
    {
      pcd_index++;
  ...
  if(laserCloudWorldRGB->size() > 0) //使用swap释放内存，避免重复分配
  {
